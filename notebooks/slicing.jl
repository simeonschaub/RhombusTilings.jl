...
function sample_path(g, npaths; tmp = Vector{Float64}(undef, length(dims)))
	...
	while v != nv(g)
		...
		s = sum(p)
		p ./= s
		v = rand(DiscreteNonParametric(n, p; check_args = false))
	end
	push!(path, v)
	return path
end
...
let
	fig = plot(t3)
	p = plot!(t3; colorrange = (1, 99), highclip = :transparent, colormap = :reds, strokewidth = 0.2, strokecolor = :red)
...
manifest_format = "2.0"
project_hash = "2fa75cd338c59f3b131b34fded02d35d85320371"
