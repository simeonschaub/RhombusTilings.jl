...
# ╔═╡ d10f9cad-ed4f-4f5a-9cb5-dba90db7f705
begin
	using Revise
	let p = dirname(pwd())
		eval(:(using Pkg; Pkg.develop(; path = $p)))
	end
	using RhombusTilings
end

# ╔═╡ 2613110c-dec1-4ba6-b7c7-d0c3c33b7d73
using WGLMakie, Bonito

# ╔═╡ 7a528214-e1f1-409b-9824-507ff0e6313f
using Graphs, SimpleWeightedGraphs, LinearAlgebra

# ╔═╡ d6680e7e-3c95-46a4-8094-95f5a0167dd7
using StaticArrays

# ╔═╡ 46bebcd9-5523-48d4-9f36-b3cdcd5df072
Page()

# ╔═╡ 73e19c0f-ade1-454a-a280-876f045810d8
t = RhombusTiling(ntuple(_ -> 1, 100))

# ╔═╡ ad761d13-0a4f-4d1b-b14c-b1a543f7b314
A = broadcast(!iszero, adjacency_matrix(t.adj))

# ╔═╡ feff307e-50de-44c1-90a3-77bbc15c637a
λ, V = eigen(Symmetric(Matrix(A)))

# ╔═╡ 3ed02263-0468-45d0-bc98-507f382f8bc7
λ′, V′ = let
	global t′ = shuffled_tiling(ntuple(_ -> 1, 100), 10^8)
	A = broadcast(!iszero, adjacency_matrix(t′.adj))
	eigen(Symmetric(Matrix(A)))
end

# ╔═╡ 08d58397-0250-4924-becd-975137034707
begin
	plot(λ)
	plot!(λ′)
	current_figure()
...
# ╔═╡ 89b51ae6-4a22-403f-950d-583853cf5c93
sum(λ)

# ╔═╡ f998cff1-7165-402b-bc5b-5ec3f5f1e722
sum(λ′)
...
function paths((; adj, vert, dims)::RhombusTiling{N}, direction = 0x01) where {N}
	idx = SVector{N - 1}(filter(!=(direction), 1:N))
	p = Array{Float64}(undef, sum(dims[idx]) + 1, dims[direction])
	x = similar(p)
	starting_points = findall(vertices(adj)) do i
        direction ∈ adj.wts[i] && all(pairs(vert[i])) do (j, x)
			if j < direction
				x == dims[j] - (j ∈ adj.wts[i])
			else
				j == direction || iszero(x)
			end
		end
    end
    ...
    for (n, i) in pairs(starting_points)
        ...
        for m in 2:size(p, 1)
			...
			p[m, n] = k
			x[m, n] = x[m - 1, n] + abs(cospi(2 * (step - Int(direction)) / N))

            _i = let i = i, i_prev = i_prev
				findfirst(adj.adj[i]) do j
	                j ∉ (0, i_prev) && get_weight(adj, i, j) == direction
	            end
			end
            _i === nothing && break
...
let
	...
	ax = Axis(fig[1, 1])
	dims = ntuple(_ -> 1, 10)
	for t in (RhombusTiling(dims), shuffled_tiling(dims, 10^8))
		A = broadcast(!iszero, adjacency_matrix(t.adj))
		plot!(ax, eigvals(Symmetric(Matrix(A))))
	end
	fig
end
...
manifest_format = "2.0"
project_hash = "3377822dabfc03c4b4014d6b7aff389f34c0c790"
...
[[deps.RhombusTilings]]
deps = ["Graphs", "Random", "SimpleWeightedGraphs", "SparseArrays", "StaticArrays"]
